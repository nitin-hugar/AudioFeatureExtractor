...
def main():
    ...
    user_input_2 = input("Download from youtube? (y)")
    if user_input == 'y':
        os.system("youtube-dl --extract-audio --audio-format wav https://www.youtube.com/watch?v=jl1mQASHc48")
    ...
    def detect_tempo(audioFilePath):
        when_beats = detect_beats(audioFilePath)
        m_res = scipy.stats.linregress(np.arange(len(beats)), when_beats)
...
# user_input = input("Split stems (y/n)?")
# if user_input == 'y':
#     os.system("conda activate splitter2")
#     os.system("spleeter separate -p spleeter:5stems -o output ../../JustGotPaid.wav")
